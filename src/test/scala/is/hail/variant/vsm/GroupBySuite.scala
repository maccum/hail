...
class GroupBySuite extends SparkSuite {
  ...
  @Test def testAggregateColumnsByKey() {
  ...
  @Test def testAggregateColumnsByKeyOfTypeStruct() {
  ...
  @Test def testAggregateColumnsByKeyInitOp() {
    // test that initop is working?
    val mt = sampleVDS.indexCols("cidx").annotateColsExpr("group" -> "sa.cidx < 3")
    val x = mt.aggregateColsByKey("{call_stats: AGG.map(g => g.GT).callStats(g => va.alleles.size())}")
    x.entriesTable().forceCount()
  }
